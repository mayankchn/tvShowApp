...
import { showIdChangedAction } from "../actions/showDetail";
...
type ShowDetailPageProps = {
  ...
  cast:Cast[];
  showId:number;
  showIdChanged:(show:Show)=>void;
  castLoaded: (cast:Cast[])=>void;
} & WithRouterProps;
...
const ShowDetailPage: FC<ShowDetailPageProps> = ({showId,show,cast,showIdChanged,castLoaded}) => {


  useEffect(() => {
    getShowDetail(showId).then((s) => {
      showIdChanged(s)
    })
    getCast(showId).then((c)=>{
      castLoaded(c)
    })
  }, [showId])
